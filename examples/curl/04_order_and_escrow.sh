...
if [[ "${PAY_METHOD,,}" == "bsc" ]]; then
  if [[ -z "${BUYER_WALLET}" ]]; then
    echo "MCP_BUYER_WALLET is required for bsc flow." >&2
    exit 1
  fi
...
sku_id="$(echo "${selected_item}" | jq -r '.sku_id // .skuId // .default_sku_id // .defaultSkuId // .sku // empty')"
...
echo "[selected] item_id=${item_id} shop_id=${shop_id} sku_id=${sku_id:-<none>}"

estimate_args="$(jq -nc \
  --argjson shipping_address_id "${shipping_address_id}" \
  --arg shop_id "${shop_id}" \
  --arg item_id "${item_id}" \
  --arg sku_id "${sku_id}" \
  ...
  '{
    ...
    item_id: $item_id,
    quantity: $quantity
  } + (if ($sku_id | length) > 0 then { sku_id: $sku_id } else {} end)'
)"

echo "== estimate_shipping =="
estimate_resp="$(call_tool "estimate_shipping" "${estimate_args}" 6)"
...
echo "== create_order =="
create_order_resp="$(call_tool "create_order" "${create_order_args}" 7)"
